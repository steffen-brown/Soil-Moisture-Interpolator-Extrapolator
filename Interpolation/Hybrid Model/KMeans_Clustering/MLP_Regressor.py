...
rawDF.to_csv("RawData/KMeans_NN.csv", index=False)
...
# print(all_models[idx])
# print(all_classes[idx])

# for l, mod in enumerate(all_models[idx]):
#     filename = f"mlp_model_{l}.joblib"
#     joblib.dump(mod, filename)

# np.random.seed(22)
# np.random.set_state(np.random.get_state())
# response_variable = np.squeeze(df[["VWC"]].to_numpy())
# external_variable = df[parameters[idx]].to_numpy()
# coordinates = df[["X", "Y"]].to_numpy()

# num_classes = nums_classes[idx]
...
# for i in range(0,len(labels_test)):
#     catagory = labels_test[i]
#     NN = all_models[idx][catagory]

#     inp = external_variable_test[i,0:len(parameters[idx])]
...
# print(r2_score(response_variable_test, predictions))
